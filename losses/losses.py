...
class CondNetCSLoss(nn.Module):
    ...
    必要な batch:
        ...
        batch["mask"]    : 任意, [B,1,H,W] or [B,H,W]
    ...
    def forward(self, pred, target, batch=None):
        ...
        loss_mean = self._loss_mean(pred, label, label14, mask)
        loss_std = self._loss_std(pred, label14, mask)
        loss_stat = self.w_mean * loss_mean + self.w_std * loss_std

        loss_rank = self._loss_rank(pred, pixel_map, label14, mask)
        loss_smooth = self._loss_smooth(pred, label14, mask)
    ...
    def _binary_erode(self, mask, iterations=1):
        x = mask.float()
    ...
    def _loss_mean(self, pred, label, label14, mask):
        device = pred.device
        vals = []

        batch_size = pred.shape[0]
        for b in range(batch_size):
            labs = torch.unique(label14[b])
            labs = labs[(labs >= 1) & (labs <= 14)]

            for lab_id in labs.tolist():
                region = (label14[b:b+1] == lab_id) & mask[b:b+1]
                if int(region.sum().item()) < 1:
                    continue

                pred_mean = pred[b:b+1][region].mean()
                true_mean = label[b:b+1][region].mean()
                vals.append(torch.abs(pred_mean - true_mean))
    ...
    def _loss_std(self, pred, label14, mask):
        device = pred.device
        vals = []
        ...
        for b in range(batch_size):
            for lab_id in self.rank_std_labels:
                ...
                cv_tgt = torch.tensor(self.target_cv[lab_id], device=device, dtype=pred.dtype)
    ...
    def _loss_rank(self, pred, pixel_map, label14, mask):
        device = pred.device
        vals = []
        ...
        for b in range(batch_size):
            for lab_id in self.rank_std_labels:
                ...
                ii, jj = self._sample_pairs(sigma.numel(), self.rank_pairs, device)
                if ii is None:
                    continue
    ...
    def _loss_smooth(self, pred, label14, mask):
        device = pred.device
        vals = []
